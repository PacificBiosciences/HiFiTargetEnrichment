...
rule deepvariant_make_examples_round1:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.{{shard}}.dv_make_examples1.tsv"
    container:
...
rule deepvariant_call_variants_gpu_round1:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.dv_call_variants1.tsv"
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    params:
...
rule deepvariant_postprocess_variants_round1:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.dv_postprocess1.tsv"
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    threads: 4
    message:
...
rule deepvariant_make_examples_round2:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.{{shard}}.dv_make_examples2.tsv"
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    params:
        vsc_min_fraction_indels="0.12",
...
rule deepvariant_call_variants_gpu_round2:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.dv_call_variants2.tsv"
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    params:
        model="/opt/models/pacbio/model.ckpt",
...
rule deepvariant_postprocess_variants_round2:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.dv_postprocess2.tsv"
    params:
...
rule deepvariant_bcftools_stats:
    ...
    benchmark:
        f"batches/{batch}/benchmarks/{{sample}}.dv_bcftools_stats.tsv"
    params:
