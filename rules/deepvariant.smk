...
rule deepvariant_make_examples_round1:
    ...
    output:
        tfrecord=temp(
            f"samples/{sample}/deepvariant_intermediate/examples/examples.tfrecord-{{shard}}-of-{config['N_SHARDS']:05}.gz"
        ),
    log:
        f"samples/{sample}/logs/deepvariant_intermediate/make_examples/{sample}.{ref}.{{shard}}-of-{config['N_SHARDS']:05}.log",
    container:
...
rule deepvariant_call_variants_gpu_round1:
    ...
    output:
        temp(
            f"samples/{sample}/deepvariant_intermediate/{sample}.{ref}.call_variants_output.tfrecord.gz"
        ),
    log:
        f"samples/{sample}/logs/deepvariant_intermediate/call_variants/{sample}.{ref}.log",
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    params:
...
rule deepvariant_postprocess_variants_round1:
    ...
    output:
        vcf=temp(
            f"samples/{sample}/deepvariant_intermediate/{sample}.{ref}.deepvariant.vcf.gz"
        ),
        vcf_index=temp(
            f"samples/{sample}/deepvariant_intermediate/{sample}.{ref}.deepvariant.vcf.gz.tbi"
        ),
        report=temp(
            f"samples/{sample}/deepvariant_intermediate/{sample}.{ref}.deepvariant.visual_report.html"
        ),
    log:
        f"samples/{sample}/logs/deepvariant_intermediate/postprocess_variants/{sample}.{ref}.log",
    container:
        f"docker://google/deepvariant:{config['DEEPVARIANT_VERSION']}"
    threads: 4
    message:
